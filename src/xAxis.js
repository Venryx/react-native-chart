...
export default class XAxis extends Component {
	...
	render() {
		...
		return (
			<View style={E(
						{flexDirection: 'row', flex: 0, backgroundColor: 'transparent', justifyContent: 'space-between',},
						style,
					)}>
				{Array(legendStepsX).fill().map((_, index)=> {
					...
					return (
						<Text key={index} style={{flex: 1, backgroundColor: 'transparent', color: axisLabelColor, fontSize: labelFontSize,
								position: "absolute", left: travelPercent * width, top: height - 20,
								width: 20, height: 20}}>
